var startStrings = @"fdgcea dgefa efabdg fdgcba dab baefc begd ebdfa db bafgdce | gfbcda cgfadb badcefg eacfgd
...
//var startStrings = "acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf";
var inputStrings = startStrings.Split("\n").Select(s =>
{
    var parts = s.Split("|");
    return (
    parts[0].Trim().Split(" ").Select(x => new string(x.ToCharArray().OrderBy(c => c).ToArray())),
    parts[1].Trim().Split(" ").Select(x => new string(x.ToCharArray().OrderBy(c => c).ToArray())));
}).ToList();

var outputValues = inputStrings.Select(x =>
{
    var mapping = getMapping(x.Item1.ToArray());
    //return mapping;
    return applyMapping(mapping, x.Item2);
}).ToList();

var totalSum = outputValues.Sum();
//var theseValues = outputValues.SelectMany(x => x).Where(x => x.Length == 2 || x.Length == 3 || x.Length == 4 || x.Length == 7).ToList();
//var c = theseValues.Count();
// See https://aka.ms/new-console-template for more information
Console.WriteLine("Hello, World!");

Dictionary<string, int> getMapping(string[] vals)
{
    var ret = new Dictionary<string, int>();

    var oneString = vals.Single(v => v.Length == 2);
    var sevenString = vals.Single(v => v.Length == 3);
    var fourString = vals.Single(v => v.Length == 4);
    var eightString = vals.Single(v => v.Length == 7);

    ret.Add(oneString, 1);
    ret.Add(sevenString, 7);
    ret.Add(fourString, 4);
    ret.Add(eightString, 8);

    var sevenSegments = new HashSet<char>(sevenString.ToCharArray());
    var oneSegments = new HashSet<char>(oneString.ToCharArray());
    var fourSegments = new HashSet<char>(fourString.ToCharArray());
    var aSegment = sevenSegments.Except(oneSegments).Single();

    var threeString = vals.Single(v => v.Length == 5 && oneSegments.IsProperSubsetOf(v)); //the one wth 5 digits that has both from onestring.
    ret.Add(threeString, 3);

    var nineString = vals.Single(v => v.Length == 6 && fourSegments.IsProperSubsetOf(v));
    ret.Add(nineString, 9);

    var zeroString = vals.Single(v => v != nineString && v.Length == 6 && oneSegments.IsProperSubsetOf(v));
    ret.Add(zeroString, 0);

    var sixString = vals.Single(v => v != nineString && v != zeroString && v.Length == 6);
    ret.Add(sixString, 6);

    var sixSegments = new HashSet<char>(sixString.ToCharArray());

    var fiveString = vals.Single(v => v != threeString && v.Length == 5 && sixSegments.IsProperSupersetOf(v));
    ret.Add(fiveString, 5);

    var twoString = vals.Single(v => v != threeString && v != fiveString && v.Length == 5);
    ret.Add(twoString, 2);
    return ret;
}

int applyMapping(Dictionary<string, int> mapping, IEnumerable<string> output)
{
    return int.Parse(string.Join("",output.Select(s => mapping[s].ToString())));
}
